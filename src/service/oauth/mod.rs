...
pub struct Service {
	...
	/// OIDC server for next-gen Matrix auth (MSC2965).
	pub oidc_server: Option<Arc<oidc_server::OidcServer>>,
}
...
impl crate::Service for Service {
	fn build(args: crate::Args<'_>) -> Result<Arc<Self>> {
		...
		let has_well_known = server.config.well_known.client.is_some();

		let oidc_server = if has_providers || has_well_known {
			if !has_providers {
				warn!(
					"OIDC server enabled (well_known.client is set) but no \
					 identity_provider configured; authorization flow will not work"
				);
			}
			info!("Initializing OIDC server for next-gen auth (MSC2965)");
			Some(Arc::new(oidc_server::OidcServer::build(&args)?))
		} else {
			None
		};
