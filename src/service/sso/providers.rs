...
use conduwuit::{Err, Result, Server, debug, err, utils::response::LimitReadExt};
...
async fn configure(
	...
	if provider.base_path.is_empty() && provider.brand == "github" {
		"login/oauth/".clone_into(&mut provider.base_path);
	}
...
async fn discover(provider: &Provider, http_client: &reqwest::Client) -> Result<JsonValue> {
	...
	let body = http_client
		...
		.await?
		.error_for_status()?
		.limit_read_text(super::RESPONSE_LIMIT)
		.await?;
